...
exports.getDevices = function (token) {
  ...
  return http.json(options).then(result => {
    if (result.errors) return new Promise().reject(result.errors[0].message)
    return result
  })
}
...
exports.sendNotification = function (ip, apiKey, text, icon, sound) {
  var options = {
  ...
  return http.post(options).then(result => {
    if (result.response.statusCode !== 201) return new Promise().reject('failure')
    return true
  })
}
