...
function onNotificationText (callback, args) {
  Homey.manager('drivers').getDriver('lametric').getSettings(args.device, (error, settings) => {
    ...
    lametric.sendNotification(settings.ipv4, args.device.apiKey, args.icontype, args.text, null, null).then(result => {
      callback(null, true)
    }).catch(callback)
  })
}
...
function onNotificationTextIcon (callback, args) {
  Homey.manager('drivers').getDriver('lametric').getSettings(args.device, (error, settings) => {
    if (error) return callback(error)
    lametric.sendNotification(settings.ipv4, args.device.apiKey, args.icontype, args.text, args.icon.code, null).then(result => {
      callback(null, true)
    }).catch(callback)
  })
}
...
function onNotificationTextIconSound (callback, args) {
  Homey.manager('drivers').getDriver('lametric').getSettings(args.device, (error, settings) => {
    if (error) return callback(error)
    lametric.sendNotification(settings.ipv4, args.device.apiKey, args.icontype, args.text, args.icon.code, args.sound.id).then(result => {
      callback(null, true)
    }).catch(callback)
  })
}
...
function onNotificationTextSound (callback, args) {
  Homey.manager('drivers').getDriver('lametric').getSettings(args.device, (error, settings) => {
    if (error) return callback(error)
    lametric.sendNotification(settings.ipv4, args.device.apiKey, args.icontype, args.text, null, args.sound.id).then(result => {
      callback(null, true)
    }).catch(callback)
  })
}
